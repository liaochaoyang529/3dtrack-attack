"""Progressive diffusion-inspired search-only 点云攻击模块。

这个文件只实现攻击流程本身，不重写 tracker。核心思想是：
1. 取出当前帧 search 点云；
2. 逐步加噪并查询 tracker，寻找刚好失败的 failure boundary；
3. 从失败点云开始逐步恢复，尽量降低肉眼可察觉形变；
4. 把恢复后的 search 点云塞回原始 tracker 输入，其余 template / memory 分支保持不变。
"""
...
class ProgressiveAttackConfig:
    ...
    这些参数由 yaml 或命令行覆盖，控制：
    - progressive noise 的最大强度和步数；
    - failure 判定阈值；
    - enhanced search-only 候选攻击；
    - recovery 阶段保留多少扰动；
    - 不可感知指标中的局部密度统计。
    """
    ...
    directional_fake_points: bool = False
    ...
    drop_ratio_max: float = 0.0
    fake_ratio_max: float = 0.0
...
class CloudState:
    ...
    points:
        当前被扰动后的点坐标。
    source_idx:
        每个点来自原始 clean search 的哪个点；-1 表示攻击插入的 fake point。
    fake_mask:
        标记哪些点是伪点。
    jitter_delta / patch_delta:
        记录真实点被 jitter 或 patch shift 移动了多少，供 recovery 阶段部分回退。
    """
...
class DriftState:
    ...
    enhanced attack 会优先复用上一帧有效方向，让攻击方向在时间上保持一致，
    避免每帧完全随机导致漂移被抵消。
    """
...
def chamfer_distance_np(pc1: np.ndarray, pc2: np.ndarray) -> float:
    """计算 clean 点云和 adversarial 点云之间的双向 Chamfer Distance。"""
...
def average_point_displacement_np(clean: np.ndarray, adv: np.ndarray, source_idx: np.ndarray) -> float:
    """统计真实保留点相对原始点的平均位移；fake point 不参与。"""
    valid = source_idx >= 0
    if not np.any(valid):
        return 0.0
...
def local_density_difference_np(clean: np.ndarray, adv: np.ndarray, k: int = 8) -> float:
    """用 kNN 平均距离变化衡量局部密度差异。

    LiDAR 点云本身不是均匀采样的。如果攻击后某个区域突然过密或过稀，
    这个指标会变大，因此它适合观察攻击是否留下局部密度异常。
    """
...
def make_initial_state(points: torch.Tensor) -> CloudState:
    """从干净 search 点云初始化可攻击、可恢复的 CloudState。"""
    n = points.shape[0]
...
def _torch_generator(device: torch.device, seed: int) -> torch.Generator:
    """创建固定 seed 的 torch 随机数生成器，保证攻击流程可复现。"""
...
def _direction_names(cfg: ProgressiveAttackConfig, drift_state: Optional[DriftState]) -> List[str]:
    """返回候选方向；drift_mode 开启时把上一帧方向放在最前面。"""
...
def _metric_attack_score(metrics: Dict[str, Optional[float]]) -> float:
    """候选攻击排序分数，越大表示 tracker 表现越差。

    GT-guided v2 中这个分数依赖评估函数提供的 IoU / center error / score。
    因此它不是最终论文指标，只用于在同一帧里挑选更强的候选扰动。
    """
...
def _target_center(points: torch.Tensor) -> torch.Tensor:
    """用 search 点云均值近似目标中心，供 patch 和方向性伪点生成使用。"""
...
def _patch_indices(points: torch.Tensor, cfg: ProgressiveAttackConfig) -> List[torch.Tensor]:
    ...
    先找靠近点云中心的候选中心点，再以每个中心点的近邻作为 patch。
    enhanced attack 会逐个测试这些 patch 的敏感性。
    """
...
def _drop_patch_state(base: CloudState, patch: torch.Tensor, cfg: ProgressiveAttackConfig) -> CloudState:
    """候选攻击：删除一个关键局部 patch。"""
    if cfg.max_drop_ratio <= 0 or patch.numel() == 0:
        return base.clone()
    state = base.clone()
    max_drop = int(round(base.points.shape[0] * cfg.max_drop_ratio))
    if max_drop <= 0:
        return state
...
def _directional_fake_state(base: CloudState, clean_points: torch.Tensor, direction: torch.Tensor, cfg: ProgressiveAttackConfig) -> CloudState:
    """候选攻击：在目标边界外沿指定方向插入伪点。

    伪点不是全局随机撒点，而是放在边界附近并沿方向拉开，
    用来诱导 tracker 往错误方向偏移。
    """
...
def _step_scale(step_id: int, cfg: ProgressiveAttackConfig) -> float:
    """把 progressive step 映射到 0~1 的相对噪声强度。"""
...
def apply_coordinate_jitter(
    ...
    """Progressive noise：对真实点逐步加入小幅坐标扰动。"""
...
def apply_point_dropping(
    ...
    """Progressive noise：按当前强度删除少量真实点。"""
...
def apply_local_density_change(
    ...
    """Progressive noise：删除局部邻域中的一部分点以制造密度变化。"""
...
def apply_local_patch_shift(
    ...
    """Progressive noise：随机选择局部 patch 并做小幅整体偏移。"""
...
def _append_points_inplace(state: CloudState, points: torch.Tensor, source_idx: int, fake: bool) -> None:
    """追加新点，并同步补齐 source/fake/delta bookkeeping。"""
    n = points.shape[0]
...
def apply_noise_step(
    ...
    每轮会根据 step_id 提升噪声强度，并按 cfg.noise_types 顺序组合多个
    噪声算子。主流程会在每一轮后查询 tracker，寻找最小失败阶段。
    """
...
def recover_state(
    ...
    """从已经攻击成功的点云开始做 progressive recovery。

    recovery 不要求恢复成原始点云，而是在保留攻击效果的前提下：
    - 减小真实点上的 jitter / patch shift；
    - 删除一部分 fake point；
    - 恢复一部分被删除的原始点。

    主流程会重新查询 tracker，只保留仍然攻击成功且不可感知分数更好的候选。
    """
...
def regularize_state_to_size(state: CloudState, sample_size: int, seed: int) -> CloudState:
    """把攻击点云重新采样到 tracker 需要的固定点数。

    攻击过程中可能删除或插入点，但 Open3DSOT 的 tracker 输入通常要求固定
    search 点数。因此在真正送入模型前，这里会做一次确定性的重采样。
    """
...
class TrackerInputAdapter:
    """屏蔽不同 tracker 输入格式差异的 search-only 适配器。

    matching 类模型（如 P2B/BAT）通常显式包含 ``template_points`` 和
    ``search_points``，这里只替换 ``search_points``。

    motion 类模型（如 M2Track）把上一帧/当前帧点拼在 ``points`` 里，
    这里约定前半是上一帧或 template-like 输入，后半是当前 search，
    因此只替换后半段。
    """

    def __init__(self, input_dict: Dict[str, torch.Tensor]):
        """识别 tracker 输入类型，并记录 search 分支固定采样点数。"""
    ...
    def get_search_points(self, input_dict: Dict[str, torch.Tensor]) -> torch.Tensor:
        """从 tracker 输入中取出当前帧 search 点云，作为攻击对象。"""
    ...
    def build_input(self, input_dict: Dict[str, torch.Tensor], adv_points: torch.Tensor) -> Dict[str, torch.Tensor]:
        """把攻击后的 search 点云写回输入，template / memory 分支不动。"""
        out = dict(input_dict)
        if self.kind == "matching":
...
def is_attack_success(metrics: Dict[str, Optional[float]], cfg: ProgressiveAttackConfig) -> bool:
    """根据当前 tracker 指标判断攻击是否成功。

    v2 的默认评估函数通常提供 GT-based IoU 和 center error；
    如果配置了 score_failure_threshold，也可以用 tracker score 作为附加条件。
    """
...
def _state_numpy(state: CloudState) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """把 CloudState 转成 numpy，方便日志、保存和不可感知指标计算。"""
    return (
...
def evaluate_state(
    ...
    """把一个 CloudState 送入 tracker 并返回攻击指标。

    注意：这里会先调用 regularize_state_to_size，因此真正进入 tracker 的
    是固定点数版本；返回的 eval_state 也对应这份实际参与推理的点云。
    """
...
def _candidate_record(
    ...
    """生成 enhanced candidate 的结构化日志记录。"""
    return {
        "stage": stage,
...
def run_enhanced_candidate_search(
    ...
    """在 progressive noise 前先做一轮 search-only 候选攻击搜索。

    该阶段会尝试：
    - critical patch drop / jitter；
    - directional fake points；
    - local patch shift。

    每个候选都会实际查询 tracker，然后用 _metric_attack_score 选择当前帧
    效果最强的起始扰动。若开启 drift_mode，会优先测试上一帧有效方向。
    """
...
def verify_search_only(input_dict: Dict[str, torch.Tensor], adv_input: Dict[str, torch.Tensor], adapter: "TrackerInputAdapter") -> Dict[str, bool]:
    ...
    matching 输入检查 template_points 和 points2cc_dist_t；
    motion 输入检查 points 前半段和 candidate_bc。
    """
...
def run_progressive_attack(
    ...
    返回值包含：
    - adv_input：可直接送入 tracker 的攻击输入；
    - clean/adv 点云和 source/fake mask；
    - progressive noise、enhanced candidate、recovery 的日志；
    - search-only invariant 检查结果；
    - 最终选择的攻击指标。

    主流程：
    1. 从输入中取出 clean search；
    2. 可选 enhanced candidate search，找敏感 patch/方向；
    3. progressive noise 找到刚好失败的 failure_state；
    4. recovery 尝试减小可见扰动；
    5. 只替换 search 点云并返回。
    """
    ...
    if cfg.enhanced_search_only:
        # 先用一批结构化候选攻击寻找更敏感的起点，再进入 progressive noise。
    ...
    if cfg.enhanced_search_only and "start_metrics" in locals() and start_metrics["attack_success"]:
        # enhanced 候选本身已经让 tracker 失败时，failure_step 记为 0。
        # v2 会继续进入 recovery，而不是直接返回这个较明显的失败点云。
    ...
    if failure_state is None:
        for step_id in range(cfg.max_noise_steps):
            # 逐步增加噪声强度，每一步都查询 tracker；第一次失败就是 failure boundary。
    ...
    if failure_state is None:
        # 如果所有噪声步都没有达到失败条件，则返回最后一轮噪声下的结果。
        best_state = states[-1]
        best_metrics, best_eval_state = evaluate_state(
    ...
    for recovery_id in range(cfg.recovery_steps):
        # recovery 会生成更接近 clean 的候选；只有仍然攻击成功且更隐蔽时才替换 best。
...
def _jsonable_metrics(metrics: Dict) -> Dict:
    """把 numpy / torch 类型递归转换成 json 可序列化对象。"""
    out = {}
