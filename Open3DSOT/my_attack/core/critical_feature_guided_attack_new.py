...
def compute_importance(features: torch.Tensor, loss: torch.Tensor):
    """Compute per-point importance from feature gradients.

    features: [B, C, N]
    returns: (scores [B, N], grads [B, C, N])
    """
    ...
    # CFG-ICCV2025 original: |grad ⊙ feature| instead of ||grad||_2
    scores = (grads * features.detach()).abs().sum(dim=1)
    max_vals = scores.max(dim=1, keepdim=True).values.clamp_min(1e-12)
    return scores / max_vals, grads
...
def _fit_surface_least_squares(points: torch.Tensor, iters: int = 100, lr: float = 0.01):
    """Fit z = ax^2 + by^2 + cxy + dx + ey + f via gradient descent (Cheng Alg.1).
    ...
    y = points[:, 1]
    z = points[:, 2]
    param = torch.zeros(6, device=points.device, dtype=points.dtype, requires_grad=True)
    for _ in range(iters):
        # f(x,y) = ax^2 + by^2 + cxy + dx + ey + f
        pred = (param[0] * x ** 2 + param[1] * y ** 2 + param[2] * x * y
                + param[3] * x + param[4] * y + param[5])
        residual = z - pred
        error = (residual ** 2).sum()
        grad = torch.autograd.grad(error, param, create_graph=False)[0]
        with torch.no_grad():
            param -= lr * grad
    ...
    if torch.isnan(result).any() or torch.isinf(result).any():
        # Fallback: plane fit z = dx + ey + f
        param_plane = torch.zeros(3, device=points.device, dtype=points.dtype, requires_grad=True)
        for _ in range(iters):
            pred = param_plane[0] * x + param_plane[1] * y + param_plane[2]
            error = ((z - pred) ** 2).sum()
            grad = torch.autograd.grad(error, param_plane, create_graph=False)[0]
            with torch.no_grad():
                param_plane -= lr * grad
        p = param_plane.detach()
        result = torch.tensor([0.0, 0.0, 0.0, p[0], p[1], p[2]], device=points.device, dtype=points.dtype)
    return result
...
def main_attack_loop(
    ...
    for step in range(attack_cfg.iters):
        ...
        # --------------------------------------------------------------
        # 4. CFG gradient for importance + L_cfg (use fusion_feature)
        # --------------------------------------------------------------
        # Fusion feature has valid gradient flow; search_feature grad is often None.
        importance_sampled, grad_fusion = compute_importance(fusion_features, l_adv)
        n_orig = adv_search.shape[1]
        ns = importance_sampled.shape[1]

        # L_cfg = Σ |grad_fusion ⊙ fusion_adv|
        L_cfg = (grad_fusion.abs() * fusion_features.detach().abs()).sum() * 0.01
