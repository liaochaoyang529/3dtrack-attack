...
def evaluate_input_against_reference(model, input_dict: Dict, reference_box, ref_bb) -> Tuple[Dict, object]:
    candidate_box = base_eval.candidate_from_model(model, input_dict, ref_bb)
    iou = base_eval.estimateOverlap(
        reference_box, candidate_box, dim=model.config.IoU_space, up_axis=model.config.up_axis
    )
...
def evaluate_one_sequence_attacked(
    ...
    for frame_id in range(frame_count):
        ...
        if frame_id == 0:
            ...
            clean_metrics_for_ref, clean_pred_box = base_eval.evaluate_input_against_gt(
                model, data_dict, this_bb, ref_bb
            )

            if attack_cfg.enabled:
                def tracker_eval_fn(candidate_input):
                    metrics, _ = evaluate_input_against_reference(model, candidate_input, clean_pred_box, ref_bb)
                    return metrics
            ...
            frame_records.append({
                ...
                "attack_reference": "clean_prediction_bbox",
                "failure_step": attack_result["failure_step"],
                "clean_metrics": attack_result["clean_metrics"],
                "clean_gt_metrics_for_eval_only": clean_metrics_for_ref,
