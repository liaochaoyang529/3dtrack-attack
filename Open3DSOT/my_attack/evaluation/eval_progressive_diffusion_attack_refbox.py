"""Progressive v2 attack with clean-prediction reference boxes.

This evaluator keeps the original v2 attack core unchanged.  The only change is
the metric reference used during attack selection:

- original v2: IoU / center error are computed against the GT box;
- refbox: IoU / center error are computed against the clean prediction box.

Final reported tracking metrics are still computed against GT.
"""
...
def evaluate_input_against_reference_box(model, input_dict: Dict, reference_box, ref_bb) -> Tuple[Dict, object]:
    candidate_box = base_eval.candidate_from_model(model, input_dict, ref_bb)
    iou = base_eval.estimateOverlap(
        reference_box,
        candidate_box,
        dim=model.config.IoU_space,
        up_axis=model.config.up_axis,
    )
    center_error = base_eval.estimateAccuracy(
        reference_box,
        candidate_box,
        dim=model.config.IoU_space,
        up_axis=model.config.up_axis,
...
def evaluate_one_sequence_attacked(
    ...
    for frame_id in range(frame_count):
        ...
        if frame_id == 0:
            ...
            clean_metrics_ref, clean_reference_box = evaluate_input_against_reference_box(
                model, data_dict, ref_bb, ref_bb
            )

            if attack_cfg.enabled:
                def tracker_eval_fn(candidate_input):
                    metrics, _ = evaluate_input_against_reference_box(
