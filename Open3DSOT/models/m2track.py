...
class M2TRACK(base_model.MotionBaseModel):
    ...
    def forward(self, input_dict, export_intermediate=False, return_point_feature=False):
        """
        Args:
            input_dict: {
            "points": (B,N,3+1+1)
            ...
            export_intermediate (bool): if True, add stageA_intermediate dict for
                offline feature/data export in inference.
            return_point_feature (bool): if True, return the global M2Track
                point_feature [B, 256] for feature-direction experiments.
        ...
        if return_point_feature:
            output_dict["point_feature"] = point_feature
        if export_intermediate:
            # Stage A exports are detached in the caller script; here we only expose tensors.
            # input_points: [B, N, 5] stacked prev/current points with timestamp and prior mask.
            # x: [B, C, N] model input after transpose (+ candidate_bc if box_aware).
            # seg_logits: [B, 2, N] foreground/background logits per point.
            # pred_cls: [B, 1, N] hard point-wise class prediction.
            # mask_points: [B, 4(+9), N] masked points (and optional box-aware channels).
            # mask_xyz_t0/mask_xyz_t1: [B, 3, N/2] masked xyz for previous/current frames.
            # point_feature: [B, 256] global feature from mini_pointnet.
            stageA = {
                ...
                "mask_xyz_t0": mask_xyz_t0,
                "mask_xyz_t1": mask_xyz_t1,
                "point_feature": point_feature,
                "aux_estimation_boxes": aux_box,
                "estimation_boxes": output_dict["estimation_boxes"],
            }
            if self.use_second_stage:
                stageA.update({
                    "mask_xyz_t0_2_t1": mask_xyz_t0_2_t1,
                    "mask_xyz_t01": mask_xyz_t01,
                    "output_offset": output_offset,
                })
            output_dict["stageA_intermediate"] = stageA
